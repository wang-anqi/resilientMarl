...
class VDNLearner(DQNLearner):
    ...
    def train_step_with(self, minibatch_data, is_adversary, target_values=None, nr_agents=None):
        ...
        print("计算并提取动作值 -state_action_values shape:", state_action_values.shape)
        total_elements = 490 * nr_agents
        if state_action_values.numel() >= total_elements:
            state_action_values = state_action_values[:total_elements]  # 截断至固定大小
        else:
            padding = total_elements - state_action_values.numel()
            state_action_values = torch.cat([state_action_values, torch.zeros(padding)], dim=0)  # 填充至固定大小

        state_action_values = state_action_values.view(-1, nr_agents)
        print("调整 -state_action_values shape:", state_action_values.shape)
        state_action_values = self.global_value(self.global_value_network, state_action_values, states)
        print("计算全局值函数 state_action_values:", state_action_values.shape)
